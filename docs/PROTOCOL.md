...
The follower app (this repository, Flutter, iOS + Android) lets a caregiver
...
**Keep in sync.** The two halves of this protocol live in two repositories:
the host paths below are in [Trio](https://github.com/nightscout/Trio), the
follower paths in this one. A change to either side is a change to both, and
neither repository's CI can see the other — so the tables here, and the
version number in `pubspec.yaml`, are the only things holding them together.
...
`AiConfig` (follower, `lib/models/pairing_bundle.dart`); the food
search request/prompt/schema: `FoodSearchManager.swift` (host) ↔
`lib/services/food_search_service.dart` (follower).
...
| Host update push | `Trio/Sources/Services/RemoteControl/FollowerHostMigration.swift` | `lib/services/host_migration_service.dart` |
